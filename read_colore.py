...
if fmt=='ASCII' :
    ra_arr,dec_arr,z_arr=read_ascii(fname)
elif fmt=='FITS' :
    ra_arr,dec_arr,z_arr=read_fits(fname)
elif fmt=='HDF5' :
    ra_arr,dec_arr,z_arr=read_hdf5(fname)

nside=64
npix=hp.nside2npix(nside)
mp=np.histogram(hp.ang2pix(nside,np.pi*(90-dec_arr)/180,np.pi*ra_arr/180),bins=npix,range=[0,npix])[0]
plt.figure(); plt.hist(z_arr,bins=100); plt.xlabel('$z$',fontsize=16); plt.ylabel('$N(z)$',fontsize=16);
